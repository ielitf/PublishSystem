package com.danggui.publishsystem.widget;
...
public class XListView extends ListView implements OnScrollListener {
	...
	// for mScroller, 滚动页眉或页脚。
	private int mScrollBack;
	private final static int SCROLLBACK_HEADER = 0;
	private final static int SCROLLBACK_FOOTER = 1;

	private final static int SCROLL_DURATION = 400; // 滚动回时间
	private final static int PULL_LOAD_MORE_DELTA = 50; // 当拉 >= 50px
	// 在底部,触发
	// 加载更多
	...
	public void setAdapter(ListAdapter adapter) {
		...
		Log.d("XListView", "setAdapter is called");
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // 高度超出，调用加载
				// more.
	...
	/**
	 * 你可以监听列表视图。OnScrollListener或这一个时,它将调用onXScrolling页眉/页脚滚动。
	 */
